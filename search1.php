...
{   
	...
	$search_content=isset($_POST['search']) ? $_POST['search'] : null ;
	$query=mysqli_query($connect,"SELECT q_id FROM tags_table WHERE (`tags` LIKE '%".$search_content."%')
        OR (`tags` LIKE '%Other%')");
	while($res=mysqli_fetch_array($query))
	{
		...
		{
			$question=$res1['questions'];
			echo '<div class="line"></div>';
			 echo '<A href=answers1.php?qid='.$res['q_id'].'>';
